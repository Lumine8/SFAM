...
def main():
    ...
    print(f"🚀 SecuADR initializing on {DEVICE}...")

    # --- MODEL ---
    model = SFAM_Adaptive(behavioral_dim=6, secure_dim=256).to(DEVICE).eval()

    # --- USER KEY (STABLE & REVOCABLE) ---
    USER_ID = "user_999"
    SALT = "salt_v1"
    user_key = generate_user_key(USER_ID, SALT, dim=256).unsqueeze(0).to(DEVICE)

    recorder = GestureCapture()

    gui_h, gui_w = 480, 640
    window_name = "SecuADR Professional"
    ...
    def mouse_callback(event, x, y, flags, param):
        ...
        elif event == cv2.EVENT_MOUSEMOVE and recorder.is_drawing:
            recorder.add_point(x, y)
    ...
    while True:
        ...
        cv2.putText(display, last_status, (20, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, last_color, 2)

        if enrolled_hash is not None:
            cv2.putText(display, "🔒 ENROLLED", (20, 440),
        ...
        if key == ord('e'):
            ...
            if s_t is not None and b_t is not None:
                with torch.no_grad():
                    enrolled_hash = model(s_t, b_t, user_key, binarize=True)
        ...
        # --- VERIFY ---
        elif key == 32 and enrolled_hash is not None:  # SPACE
            s_t, b_t, _ = recorder.process_gesture(device=DEVICE)
            if s_t is not None and b_t is not None:
                with torch.no_grad():
                    live_hash = model(s_t, b_t, user_key, binarize=True)

                # Hamming distance
                dist = torch.mean((live_hash != enrolled_hash).float()).item()

                if dist < 0.15:
                    last_status = f"GRANTED ({dist:.2f})"
                    last_color = (0, 255, 0)
                    print(f"✅ Access Granted ({dist:.2f})")
                else:
                    last_status = f"DENIED ({dist:.2f})"
                    last_color = (0, 0, 255)
                    print(f"❌ Access Denied ({dist:.2f})")
            else:
                last_status = "Gesture too short!"
